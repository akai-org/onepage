...
module.exports = [__webpack_require__(5), __webpack_require__(6), __webpack_require__(7), __webpack_require__(8), __webpack_require__(9), __webpack_require__(10), __webpack_require__(11), __webpack_require__(12), __webpack_require__(13)];
...
var pageRenderer = __webpack_require__(4);
var components = __webpack_require__(1);

document.addEventListener("DOMContentLoaded", function () {
  var isDev = $('title').text() === "Webpack App";

  if (isDev) {
    $.getJSON("./../config.json").done(function (config) {
      $("body").append(pageRenderer.render(config));
    ...
    var pageComponents = components.filter(function (component) {
      return $(component.selector).length;
    });
...
var pageRenderer = function () {
  ...
  var registerComponents = function registerComponents() {
    var components = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : Components;
  ...
  var render = function render(data) {
    ...
    title = pageTitle;
    var compiledPageComponents = components.map(function (componentData) {
      var name = componentData.name,
          data = componentData.data;

      var component = allComponents.find(function (componentData) {
        return componentData.name === name;
      });
...
var Home = function () {
  ...
  var headingTitle = function headingTitle(title) {
    if (title) {
  ...
  var headingSubtitle = function headingSubtitle(subtitle) {
    if (subtitle) {
  ...
  var background = function background(backgroundUrl) {
    if (typeof backgroundUrl === "string") {
      var ext = backgroundUrl.substring(backgroundUrl.length - 4, backgroundUrl.length);
  ...
  var headingDescription = function headingDescription(text) {
    if (text) {
  ...
  var button = function button(btn, cssClass) {
    if (btn === undefined) {
      return '';
    }
  ...
  var scroll = function scroll(btn) {
    if (btn === undefined) {
      return '';
    }
  ...
  api.compile = function (data) {
    var title = data.title,
        ...
        scrollButton = data.scrollButton;
...
var About = function () {
  ...
  var header = function header(title, subtitle) {
    var compiledTitle = '';
    var compiledSubtitle = '';
  ...
  var photo = function photo(image) {
...
var DateAndVenue = function () {

  var api = {};

  var dateColumn = function dateColumn(date, time) {
    if (date && time) {
  ...
  var venueColumn = function venueColumn(venue, details) {
    if (venue && details) {
  ...
  var calendar = function calendar(data) {
    var google = function google(link) {
      if (link) {
...
var Events = function () {
  ...
  var informationList = function informationList(list) {
  ...
  var informationText = function informationText(text) {
    if (text) {
  ...
  var expandableDetails = function expandableDetails(text, list) {
    if (text || list) {
  ...
  var timeColumn = function timeColumn(time) {
    if (time) {
  ...
  var authorName = function authorName(name) {
    if (name) {
  ...
  var mainDescription = function mainDescription(title, author) {
    if (title) {
  ...
  var compileEvents = function compileEvents(events) {
...
var Contact = function () {
  ...
  var heading = function heading(title) {
    if (title) {
...
var CollaborationPricing = function () {
  ...
  api.compile = function (data) {
    ...
    var benefits = collaboration.benefits,
        plans = collaboration.plans;
    var name = benefits.name,
        categories = benefits.categories;

    var compiledCategories = categories.map(function (category) {
      return '<li>' + category + '</li>';
    }).join("\n");
    var compiledPlans = plans.map(function (plan) {
      var name = plan.name,
          price = plan.price,
          period = plan.period,
      ...
      var compiledPlanBenefits = benefits.map(function (benefit, index) {
  ...
  var resizeRow = function resizeRow(row) {
    ...
    for (var _j = 0; _j < row.length; _j++) {
      $(row[_j]).height(maxHeight);
    }
  };

  var resizeCells = function resizeCells() {
    var $benefits = $('.benefits-list li');
    var $plans = $('.plan-list li');

    for (var i = 0; i < $benefits.length; i++) {
      var row = [];
      row.push($benefits[i]);

      for (var j = 0; i + j * $benefits.length < $plans.length; j++) {
  ...
  var resizeTable = function resizeTable() {
    var $headings = $('.collaboration-plans .heading');
    resizeRow($headings);
    resizeCells();
  };
...
var Footer = function () {
  ...
  var socialLinks = function socialLinks(socials) {
  ...
  var compileSocials = function compileSocials(socials) {
    if (socials) {
  ...
  var contact = function contact(email) {
    if (email) {
  ...
  var copyright = function copyright(title, copy) {
    if (title && copy) {
...
var Gallery = function () {
  ...
  api.componentReady = function () {
    ...
    $slider.mouseenter(pauseSlider).mouseleave(startSlider);
...
var Roles = function () {
  ...
  var compileSections = function compileSections(sections) {
  ...
  var contact = function contact(cta, contactButton) {
